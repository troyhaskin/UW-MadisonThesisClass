...
\newcommand{\IfCounterIsGlobal}[3]{
    \ifltxcounter{#1}%  LaTeX counters are global counters
        {#2}
        {\ifcscounter{#1}
            {#3}
...
\newcommand{\StepCounter}[1]{
    \IfCounterIsGlobal{#1}
        {\stepcounter{#1}}
        {\expandafter\advance\csname#1\endcsname 1}
}
\newcommand{\AddToCounter}[2]{
    \IfCounterIsGlobal{#1}
        {\addtocounter{#1}{#2}}
        {\expandafter\advance\csname#1\endcsname #2}
}
...
\newcommand{\HashSet}[3]{% #1 = hash name, #2 = key name, #3 = value
    ...
    \IfCommandExists{#1#2\HashSuffix}{
        ...
        \ArrayPush{#1\HashSuffixKeys}{#2}%      Push key to Array storing all keys for this hash
...
\newcommand{\HashGetValueByIndex}[2]{% #1 = hash name, #2 = index value
    \IfCommandExists{#1\HashSuffix}{%
        \IfCommandExists{#1#2\HashSuffix}{%
            \HashGet{\ArrayGet{#1\HashSuffixKeys}{#2}}%
        }{%
            \UWMad@ClassWarning{There is no key '#2' set for hash '#1'}%
        }%
    }{%
